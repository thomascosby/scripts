...
if [ -f $LOCK_FILE ]; then
	TIMESTAMP=$(date)
        ...
        mail -s "[ANTHEM SF RSYNC (OPERATIONS LOCAL)] - ERROR]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE
...
echo "$MAILTEXT" | mail -s "[ANTHEM SF RSYNC (OPERATIONS LOCAL)]" "$RECIPIENTS"
