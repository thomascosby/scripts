...
if [ -f $LOCK_FILE ]; then
	TIMESTAMP=$(date)
        ...
        mail -s "[ANTHEM SF RSYNC (Clients LOCAL)] - ERROR]" $RECIPIENTS < $DAILY_LOG
	exit
fi
touch $LOCK_FILE
...
then
   $RSYNC -avhu --stats --delete --max-delete=51000 --exclude-from '/Library/Scripts/Schawk/asf_rsync_exclude_list.txt' $SOURCEVOL $DESTVOL 2>&1 >> $DAILY_LOG
else 
...
echo "$MAILTEXT" | mail -s "[ANTHEM SF RSYNC (Clients LOCAL)]" "$RECIPIENTS"
